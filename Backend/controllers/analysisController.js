...
export const analyzeCode = async (req, res) => {
  ...
  try {
    ...
    const explanationMatch = sanitized.match(/explanation[:\-]?\s*(.*)/i);
    ...
    const explanation = explanationMatch?.[1]?.trim() || "Explanation not available.";

    res.json({ timeComplexity, spaceComplexity, explanation });
  } catch (error) {
